...
struct CIInventoryTests {
    ...
    static func sample() throws -> ContinuousIntegration.Inventory.Document {
        try ContinuousIntegration.Inventory.Document(
    ...
    struct Topology {
        @Test func `the inventory describes one hop, not three`() throws {
            #expect(ContinuousIntegration.Inventory.Document.callerHops == 1)
            #expect(ContinuousIntegration.Inventory.Document.schemaVersion == 2)
        }

        @Test func `the verdict resolves to the one required check context`() throws {
            #expect(
                ContinuousIntegration.Inventory.Aggregate.checkContext
                    == "ci / matrix / ci-ok"
            )
            #expect(
                ContinuousIntegration.Inventory.Aggregate.checkContext
    ...
    struct Derivation {
        ...
        @Test func `the plan delegates its leg vocabulary to its Swift owner`() throws {
            ...
            #expect(!ContinuousIntegration.Inventory.Plan.fullTierLegs.isEmpty)
        }

        @Test func `every full-tier leg names a job the universal actually declares`() throws {
            let universal = try CIInventoryTests.sample().universal
            let declared = Set(universal.jobs.map(\.id))
            for leg in ContinuousIntegration.Inventory.Plan.fullTierLegs {
    ...
    struct `Edge Case` {
        @Test func `a workflow with no jobs is refused, not inventoried as empty`() {
            #expect(throws: ContinuousIntegration.Inventory.Error.noJobs) {
                try ContinuousIntegration.Inventory.Document(
        ...
        @Test func `a workflow with no ci-ok is refused by name`() {
            #expect(throws: ContinuousIntegration.Inventory.Error.missingJob("ci-ok")) {
                try ContinuousIntegration.Inventory.Document(
        ...
        @Test func `a workflow with no plan is refused by name`() {
            #expect(throws: ContinuousIntegration.Inventory.Error.missingJob("plan")) {
                try ContinuousIntegration.Inventory.Document(
                    universalWorkflow: "jobs:\n  ci-ok:\n    runs-on: ubuntu-latest\n"
                )
            }
        ...
        @Test func `a bare needs scalar is the same DAG as a one-element list`() {
            ...
            #expect(
                ContinuousIntegration.Inventory.Universal.wave(
                    of: "a",
                    needs: waves,
                    cache: &cache
                ) == 0

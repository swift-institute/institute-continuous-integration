...
/// `ContinuousIntegration.Validation.Registry` runs against the
...
struct InstituteValidationCorpusTests {
    ...
    static func run(in corpus: Validation.Corpus) throws -> (outcomes: [Outcome], unowned: [String])
    {
        ...
        for directory in try corpus.ruleDirectories() {
            guard
                let rule = ContinuousIntegration.Validation.Registry
                    .rule(forCorpusDirectory: directory),
                let validator = ContinuousIntegration.Validation.Registry
                    .validator(for: rule)
            else {
    ...
    @Test func `no two validators claim the same rule`() {
        ...
        for validator in ContinuousIntegration.Validation.Registry.validators {
    ...
    @Test func `every registered rule resolves back to its validator`() {
        for rule in ContinuousIntegration.Validation.Registry.rules {
            #expect(
                ContinuousIntegration.Validation.Registry.validator(for: rule) != nil
            )
        }
    }

...
struct CIValidationSchemaCorrespondenceTests {
    ...
    static func scenario(
        _ kind: String,
        _ name: String
    ) -> ContinuousIntegration.Validation.SchemaCorrespondence {
    ...
    struct Integration {
        ...
        @Test func `a missing schema is a defect, not a clean run`() {
            let validator = ContinuousIntegration.Validation.SchemaCorrespondence(
    ...
    @Suite
    struct Unit {
        typealias Validator = ContinuousIntegration.Validation.SchemaCorrespondence

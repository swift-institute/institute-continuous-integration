...
/// The family-routing behaviour of `ContinuousIntegration.Validation.Readme` that the
...
struct CIValidationReadmeTests {
    let validator = ContinuousIntegration.Validation.Readme()
    ...
    struct Registry {
        @Test func `every corpus directory resolves to a registered rule`() {
            for directory in ["readme-008", "readme-013", "readme-017", "readme-026"] {
                #expect(
                    ContinuousIntegration.Validation.Registry.rule(

...
extension Repository.Policy.Uniformity.Wave {
    public static func enumerate<C: Client>(
        ...
        guard !subjects.isEmpty else {
            throw .population("active fleet enumeration produced zero eligible subjects")
        }
        // A zero-subject organization is a typed census fact, never a
        // preflight matrix row (wave run 31817391995, swift-riscv).
    ...
    static func shape<C: Client>(
        client: C,
        repository: String,
        head: String
    ) async throws(Error) -> Shape {
        let gitignore = try await calling {
            try await client.shapeFile(repository, path: Shape.gitignorePath, head: head)
        }
        let swiftlint = try await calling {
            try await client.shapeFile(repository, path: Shape.swiftlintPath, head: head)
        }
        let swiftFormat = try await calling {
            try await client.shapeFile(repository, path: Shape.swiftFormatPath, head: head)
        }
        let dependabot = try await calling {
            try await client.shapeFile(repository, path: Shape.dependabotPath, head: head)
        }
        return Shape(
            gitignore: gitignore,
            swiftlint: swiftlint?.blob,
            swiftFormat: swiftFormat?.blob,
    ...
    private static func measure<C: Client>(
        ...
        let shape = try await shape(client: client, repository: repository.fullName, head: head)
    ...
    static func calling<T>(
        ...
        try await Repository.Policy.Caller.Wave.calling(body)
    }
}

...
extension ContinuousIntegration.Validation.SkillHygiene {
    ...
    /// sanctioned.
    struct Prose {
        private typealias Rules = ContinuousIntegration.Validation.SkillHygiene
        ...
        func proseFindings(
            ...
            var findings: [GitHub.ContinuousIntegration.Validation.Finding] = []
            for (offset, line) in ContinuousIntegration.Validation.SkillHygiene.lines(
                of: text
            ).enumerated() {
                let number = offset + 1
                    ...
                    findings.append(
                        finding(
                            ...
                            "\(relative):\(number): '\(token)' is not in "
                                + "\(ContinuousIntegration.Validation.SkillHygiene.sanctionedReferences). "

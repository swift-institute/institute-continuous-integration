...
extension ContinuousIntegration.Bootstrap.Manifest {
    public enum VerificationError: Swift.Error, Sendable, Equatable {
        case identity(ContinuousIntegration.Bootstrap.Identity.ValidationError)
    ...
    public func verify(
        against expected: ContinuousIntegration.Bootstrap.Identity,
        bytes: (String) -> [Byte]?
    ) throws(VerificationError) {
        do throws(ContinuousIntegration.Bootstrap.Identity.ValidationError) {

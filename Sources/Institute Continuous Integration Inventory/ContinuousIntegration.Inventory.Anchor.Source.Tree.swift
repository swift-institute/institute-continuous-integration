...
extension ContinuousIntegration.Inventory.Anchor.Source {
    ...
    public struct Tree: Sendable, Equatable {
        ...
        public let oid: ContinuousIntegration.Inventory.Anchor.Revision

        public init(
            path: String,
            oid: ContinuousIntegration.Inventory.Anchor.Revision
        ) {
            self.path = path
            self.oid = oid
        }
